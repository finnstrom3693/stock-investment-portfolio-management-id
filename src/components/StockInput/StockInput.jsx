...
const UploadJSONModal = ({ isOpen, onClose, onUpload }) => {
...
const StockInput = () => {
  ...
  const [uploadAppendModalOpen, setUploadAppendModalOpen] = useState(false);
  ...
  const handleUploadAppendJSON = (file) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      const jsonData = JSON.parse(e.target.result);
      setStocks([...stocks, ...jsonData]);
      setShowUploadModal(false);
      alert('JSON data appended successfully!');
    };
    reader.readAsText(file);
  };
  ...
  return (
    <div>
      ...
      <div className="container">
        <div className="row mb-3">
            <div className="col-auto">
            <button className="btn btn-success my-2" onClick={handleSaveLocalStorage}>
                Save to Local Storage
            </button>
            </div>
            <div className="col-auto">
            <button className="btn btn-danger my-2" onClick={handleDeleteLocalStorage}>
                Delete Local Storage Data
            </button>
            </div>
            <div className="col-auto">
              <button className="btn btn-primary my-2" onClick={handleDownloadJSON}>
                Download JSON
              </button>
            </div>
            <div className="col-auto">
              <button className="btn btn-primary my-2" onClick={() => setShowUploadModal(true)}>
                Upload JSON
              </button>
            </div>
            <div className="col-auto">
              <button className="btn btn-primary my-2" onClick={() => setUploadAppendModalOpen(true)}>
                Append Upload JSON
              </button>
            </div>
            <div className="col-auto">
              <Link to="/piechart" className="btn btn-info text-light my-2">
                Stock Piechart
              </Link>
            </div>
        </div>
      ...
      <div className={`${isMobile ? '' : 'container'}`}>
        <div className="ms-3 text-uppercase fw-bold title-table2">My Portfolio</div>
        <div className="d-flex justify-content-between align-items-center my-2">
          <div className="ms-3 text-uppercase fw-bold title-table">My Portfolio</div>
          <span className='display-acc mx-2'>Total Stock : {totalStockCount}</span>
          <span className='display-acc mx-2'>Total Investment: Rp.{totalInvestment.toFixed(0)}</span>
          <span className='display-acc mx-2'>Total Shares: {totalShares.toFixed(0)}</span>
          <div className="ms-auto"><input type="text" onChange={handleSearch} /></div>
        </div>
        <span className='display-acc2'>Total Stock : {totalStockCount}</span>
        <span className='display-acc2'>Total Investment: Rp.{totalInvestment.toFixed(0)}</span>
        <span className='display-acc2'>Total Shares: {totalShares.toFixed(0)}</span>
        <table className={`${isMobile ? 'table-responsive-sm bg-light py-2 px-2 rounded' : 'table'}`}>
          <thead>
            <tr>
          ...
          <tbody>
            {stocks.map((stock, index) => (
              <tr key={index}>
                ...
                <td>
                  <button className="btn btn-primary me-2" onClick={() => handleEditStock(index)}><i class="fa-solid fa-pen-to-square"></i></button>
                  <button className="btn btn-danger me-2" onClick={() => handleDeleteStock(index)}><i class="fa-solid fa-trash"></i></button>
                  <Link to={`/stock/${stock.stockIndex}`} className="btn btn-secondary"><i class="fa-solid fa-eye"></i></Link>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {/* Append Upload JSON Modal */}
      <UploadJSONModal
        isOpen={uploadAppendModalOpen}
        onClose={() => setUploadAppendModalOpen(false)}
        onUpload={handleUploadAppendJSON}
      />
    </div>
  );
};
